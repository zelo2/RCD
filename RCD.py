...
class RCDNet(nn.Module):
    ...
    def forward(self, user, item, q, indicator):
        '''
        :param user: [batch_size] user_id
        :param item: [batch_size] item_id
        :param q: Q-matrix [item_num, skill_num]
        :param indicator: [stu_num, item_num] 1-Train, 0-Test, Nan-Student does not interact with
        :return: Performance prediction, SKill proficiency of students
        '''
        batch_size = len(user)
